    ...
    >
      <div className="relative w-full">
        ...
        <div className="mr-10 flex justify-end gap-2">
          ...
          >
            <IconArrowNarrowRight className="h-6 w-6 text-accent" />
          </button>
        </div>
      </div>
    </CarouselContext.Provider>
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-background border border-white/10 md:h-[40rem] md:w-96 hover:border-accent/50 transition-colors"
      >
        <div className="pointer-events-none absolute inset-x-0 top-0 z-30 h-full bg-gradient-to-b from-black/50 via-transparent to-transparent" />
        <div className="relative z-40 p-8">
          <motion.p
            layoutId={layout ? `category-${card.category}` : undefined}
            className="text-left font-sans text-sm font-medium text-accent md:text-base"
          >
            {card.category}
          </motion.p>
          <motion.p

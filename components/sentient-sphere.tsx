...
export function SentientSphere() {
  ...
  useEffect(() => {
    ...
    // Turn off the heavy 3D canvas on small screens or when the user prefers reduced motion
    if (prefersReducedMotion || isSmallScreen) {
      setShouldRender(false)
    }
  }, [])

  // Initial skeleton while mounting to avoid hydration jank
  if (!mounted) {
    return (
  ...
  // Lightweight fallback: static blurred orb instead of a live WebGL scene
  if (!shouldRender) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="w-64 h-64 md:w-80 md:h-80 rounded-full bg-gradient-to-br from-white/20 via-white/5 to-transparent blur-3xl border border-white/10" />
      </div>
    )
  }
